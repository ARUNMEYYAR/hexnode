...
document.querySelectorAll('.tab').forEach(tab => {
    tab.addEventListener('click', function () {
        ...
        if (targetContent) {
            ...
            setTimeout(() => {
                targetContent.classList.add('animate-content');
            }, 50); // Small delay to trigger animation
        }
    });
});

// JavaScript function to change the image on the left when a FAQ item is clicked
function changeImage(imageName) {
	var imagePath = 'assets/images/banner/' + imageName; // Modify path based on your structure
	document.getElementById('faq-image').src = imagePath;
  }